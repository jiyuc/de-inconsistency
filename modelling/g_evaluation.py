...
if __name__ == '__main__':
    ...
    # dev dataset
    test_dataset_integrator = Integration(test_path, bg=has_bg, generif=use_generif, type=type_mapper(test_path),
                                         flag='test')
    test_x, test_labels, test_node_embedding = test_dataset_integrator.forward()
    test_encodings = Encode(test_x).forward()
    test_dataset = DatasetBert(test_encodings, test_node_embedding, test_labels)
    print(f'{test_dataset.__len__()} instances in dev set')
