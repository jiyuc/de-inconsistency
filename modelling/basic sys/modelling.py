...
if __name__ == '__main__':
    ...
    train_x, train_labels = train_dataset_integrator.forward()
    train_encodings = Encode(train_x).forward()
    train_dataset = DatasetBert(train_encodings, train_labels)
    print(f'{train_dataset.__len__()} instances in training set')

    # dev dataset
    dev_dataset_integrator = Integration(dev_path, bg=has_bg, generif=use_generif, type=type_mapper(dev_path))
    dev_x, dev_labels = dev_dataset_integrator.forward()
    dev_encodings = Encode(dev_x).forward()
    dev_dataset = DatasetBert(dev_encodings, dev_labels)
