...
class Integration:
    ...
    def __init__(self, goa_record, bg=False, generif=False, type='co'):
        ...
        if bg:
            ...
            try:
                self.go_terms = [r.split('\t')[-2] + ' ' + go_index[r.split('\t')[-2]] + '[SEP] [MASK]' for r in goa_records]
            except KeyError:
                for r in goa_records:
                    try:
...
class Preprocessing:
    ...
    def forward(self):
        ...
        gene_info = self.gene_symbol

        # if num_of_cls == 4:  # term inconsistency
        gene_conditioned_evidence = ' [SEP] '.join((evidence, gene_info))
